"""
File for generating temporally comparable data.
...
default_time_deltas = (np.arange(0, 6*60, 15) + 15) * 60
...
def center_data(data1, data2, time_deltas=default_time_deltas):
    ...
    for item_iter in data1.iterrows():
        ...
        for time_delta in time_deltas:
            item = item_iter[1]
            item_time = item.Time
            min_time = item_time - pd.Timedelta(seconds=time_delta / 2)
            max_time = item_time + pd.Timedelta(seconds=time_delta / 2)
...
def rad_center_data(data1, data2, time_deltas=default_time_deltas):
    time_deltas = sorted(time_deltas, reverse=True)
    """
    Resamples RAD and TSI data.
    """
    output = []

    ds = ds = pd.merge(data1, data2, on="Time", how="outer")

    i = 0
    for delta in time_deltas:
        print("\r%s/%s   " % (i, len(time_deltas)), end="")
        dx = ds.resample(str(delta) + "s", on="Time").mean()
        dx["Timedelta"] = delta
        output.append(dx)
        i += 1
...
def process_all(modis_data, path_dict=default_path_dict,
    ...
    """
    Routine for processing everything at once.
    """
    data1 = modis_data
...
def process_all_lazy():
    """
    Routine for processing everything at once using default parameters.
    """

    print("Resampling TSI and RAD Data")
    tsi_path = default_path_dict["tsi"]
    rad_path = default_path_dict["rad"]
    data1 = pd.read_csv(tsi_path, na_values=["--"], parse_dates=["Time"])
    data2 = pd.read_csv(rad_path, na_values=["--"], parse_dates=["Time"])
    ds = rad_center_data(data1, data2)
    ds.to_csv(os.path.join(proc_path, "tsi-centered-rad.csv"))
...
def average_modis_path(modis_path, minutes=5):
    """
    Updates given MODIS processed file to have consistent timestamps
    for different spatial deltas.
    """
    modis_data = pd.read_csv(modis_path, na_values=["--"],
                             parse_dates=["Time"])
    data = modis_data
