...
sitelist = ['MN']
# sitelist=['S1','S2','S3','S4','S5','S6','S7','S8','S9']

# Define the seasons to be run. Season definition dates in physconstants.py
seasonlist = ['DRY', 'WET']

# Define the half size of a box around each site location. Nominal pixel
# size is 4km, so a box size of 12 pixels means ~96 x 96 km^2 box.
boxsizelist = ['50']
...
#
dataroot = os.path.expanduser("~/dados-ic/GOES/")

# Define the start and end years to be used in the calculations. These are
# subfolders in the dataroot folder.
startyear = 2015
endyear = 2015

# Define where the results will be stored
# outroot='/home/acorreia/Desktop/phase/output/18z-isccp-hotwater/'
outroot = os.path.expanduser("~/cod_acorreia/output/")

# Define the name of the output file. This should be only the "rootname",
# like 'analysis-'. The final filenames will be like 'analysis-2010.csv',
# 'analysis-2011.csv', etc.
outfileroot = outroot + 'allsites-isscp-hotwater-'

# Define the name for a 2D histogram to be plotted at the end
png2dhistogram = outroot + 'Temp-Reff_2D.png'

# Define the location of the scripts that are part of the processing chain
# Don't change this unless you know what you are doing.
pexec = 'python3'
codedir = os.path.expanduser("~/cod_acorreia/")
...
for fdir in range(startyear, endyear + 1):
    ...
    while (i < len(sitelist)):
        ...
        while (j < len(boxsizelist)):
            cmd1 = pexec + ' ' + p1 + ' ' + datadir + ' ' + outdir + '/' + \
                sitelist[i] + ' ' + sitelist[i] + ' ' + \
                boxsizelist[j] + ' phase=' + phaseopt
            cmd2 = pexec + ' ' + p2 + ' ' + outdir + '/' + \
                sitelist[i] + ' ' + outdir + '/' + sitelist[i]
            ...
            while (k < len(seasonlist)):
                cmd4 = pexec + ' ' + p4 + ' ' + outdir + '/' + sitelist[i] + '/' + seasonlist[
                    k] + ' ' + outdir + '/' + sitelist[i] + '/' + seasonlist[k] + ' ' + seasonlist[k] + ' ' + sitelist[i]
                cmd5 = pexec + ' ' + p5 + ' ' + outdir + '/' + sitelist[i] + '/' + seasonlist[
                    k] + ' ' + outdir + ' ' + seasonlist[k] + '-' + sitelist[i] + '.csv ' + seasonlist[k] + ' ' + sitelist[i]
                print()
                print(cmd4)
                os.system(cmd4)
                print()
    ...
    while (i < len(sitelist)):
        k = 0
        while (k < len(seasonlist)):
            cmd7 = pexec + ' ' + p7 + ' ' + outdir + '/' + sitelist[i] + '/' + seasonlist[
                k] + ' ' + outfilename + ' ' + outroot + sitelist[i] + '-' + seasonlist[k] + '-reffready.csv'
            print()
            print(cmd7)
            os.system(cmd7)
            k = k + 1
...
cmd9 = pexec + ' ' + p9 + ' ' + outfileroot + \
    'reffready.csv ' + mylut + ' ' + png2dhistogram
print()
print(cmd8)
os.system(cmd8)
print()
